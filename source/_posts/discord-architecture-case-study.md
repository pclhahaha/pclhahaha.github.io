...
Discord 的单服极限在 MidJourney 的冲击下达到了新高度。MidJourney 官方服务器的并发用户突破了 **1100 万**，远超之前最大的公会（Roblox 和 Fortnite 约 100 万成员）。Discord 工程团队此前认为 100 万成员已接近单服极限——这一假设被彻底打破。

为了应对这一规模，Discord 采用了多语言协同策略：**Rust 处理热点路径**上的计算密集型任务，而 Elixir 继续负责 95% 的业务逻辑（连接管理、消息路由）。当单语言方案触及性能上限时，用不同语言覆盖不同复杂度层级，是工程实用主义的典范。
