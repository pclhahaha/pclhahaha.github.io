...
### 8.1 区域失效守护进程 (Regional Invalidation Daemon)

Meta 面临一个独特的挑战：当用户的数据在一个数据中心被更新后，其他区域的数据中心需要及时感知缓存失效。Meta 的解决方案是部署专门的 **mcsqueal** 守护进程。

这个守护进程监视 MySQL 的 commit log（类似 binlog 监听），当检测到数据库更新时，它向对应区域的 Memcached 集群发送失效指令。关键设计在于：mcsqueal 先等待数据库的主从复制完成后，才发送失效到副本区域——这避免了"先失效缓存、后读到旧数据"的竞争条件。Meta 选择了**可以容忍短暂读到旧数据**的最终一致性，以换取系统的可扩展性。

### 8.2 Slab 分配器自动调整

标准 Memcached 的 Slab 分配器需要手动调整 slab 大小比例。Meta 修改后的版本能根据实际请求模式**动态自适应调整 slab 分配**——当观察到某类大小的对象访问频率激增时，自动为其分配更多内存页，最大化缓存命中率而无需人工干预。
